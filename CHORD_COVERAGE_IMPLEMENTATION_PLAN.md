...
This plan expands the deterministic chord engine so the LH Stradella and RH CBA views can safely handle the common-but-currently-missing chord symbols we expect to encounter in mainstream piano/vocal books, guitar lead sheets, and the planned score-photo lookup feature.
...
- `src/components/ChordBadge.tsx`, `StradellaMiniCard.tsx`, `StradellaGrid.tsx`, `MiniGripDrawer.tsx` — consumers that must remain truthful when a voicing has no chord button.
- `src/lib/capo/capo.test.ts`, `src/lib/cba/cba.test.ts`, `tests/unit/stress_m2_theory.test.ts`, and parser tests — existing test style and regression coverage.
...
Use these exact names so downstream switches remain readable and consistent with existing names such as `sevenFlatNine` and `sevenSharpEleven`.

### 2. Fix classification order, not just regex acceptance

`src/lib/capo/transposition.ts` currently recognizes several strings syntactically but collapses them into another quality. The implementation must classify the locked vocabulary semantically.
...
`src/lib/parser/twoline.ts::isChordToken()` has a separate chord-token grammar. Extend it to accept all required aliases above, including `m(maj7)` forms and parenthesized `7(#5)` / `7(#9)` forms.

Do not solve this by accepting every token starting with A-G. Invalid text must not begin producing chord badges.

Add regression tests asserting that every locked spelling is accepted by `isChordToken()` and classified to the expected `ChordQuality` by `parseChord()`.

### 4. Preserve slash-bass semantics

Every new family must still allow the existing `/Bass` suffix where syntactically meaningful, for example:
...
The specific photographed-score-style case `Em(maj7)/D#` must be a named regression test. It must parse as root `E`, quality `minorMajor7`, bass `D#` and must not become an E-major/major-7 chord internally.
...
The engine already uses practical four-note voicings for some large extensions, so `dominant11` and `minor11` must remain at four RH buttons. Do not try to place five or six simultaneous notes on the CBA grid.
...
`generateCbaGrip()` currently assumes triads/four-note chords when choosing the display fingering string. For a two-note `power5` grip:

- return exactly two button coordinates;
- use the two-note fingering pattern already naturally produced by the coordinate assignment (`1-2`) unless a mathematical/ergonomic review demonstrates a better stable rule across all 12 roots;
- ensure the displayed `fingeringPattern` matches the actual button `finger` values rather than incorrectly showing `1-2-4`.
...
- `minorMajor7`: root bass + root minor chord (`offset 0`, minor row); explanation says RH adds major 7.
...
`src/lib/stradella/grooves.ts` currently invents a root-major chord name when `voicing.chordButton` is missing. That is unsafe once bass-only qualities are deliberate.

Change groove solving so a voicing with no `chordButton` does **not** produce fake chord/chop steps. For this milestone the simplest correct behavior is:
...
`StradellaGrid`, `StradellaMiniCard`, `ChordBadge`, and `MiniGripDrawer` already use optional `chordButton` in many places. Confirm with tests/visual audit that bass-only voicings:
...
For each family, test at least the canonical C-root spelling plus aliases. Include Unicode `♯` where relevant. Assert `quality`, normalized root, extension, slash bass, and formatted raw chord where appropriate.
...
For every new quality, assert exact pitch classes for root C. Then run a 12-root loop checking interval transposition modulo 12.
...
Assert exact recipes for root C and at least one flat/sharp root. Add 12-root loop coverage for column/range invariants where practical.
...
Run the existing parser, capo/enharmonic, Stradella, CBA, rendering and stress tests. Existing supported qualities must not change their pitch sets or LH recipe unexpectedly.
