...
| **FLOW-04** | **Accordion View Switcher**                            | View Modes:<br>• `🪗 LH` (Left Hand Stradella Bass)<br>• `🔘 RH` (Right Hand CBA C-System Treble)<br>• `🎸 Dual` (Guitar Chords + Accordion Voicing)                                                                                                          | • LH mode renders clean sounding chord, counter-bass slash chord (`C/B_`), or compound recipe (`C+em`)<br>• RH mode renders Sounding chord name + fingering `[1-2-4]`<br>• Dual mode renders stacked chord badge with original root and accordion button<br>• Zero document scroll jump during view mode toggle |
| **FLOW-05** | **Mini-Grip Drawer Natural Height & Touch Ergonomics** | • ChordBadge tap trigger<br>• Modal backdrop tap dismiss<br>• Header close button dismiss<br>• Stradella & CBA interactive grid diagrams                                                                                                                      | • ChordBadge touch target hitbox >= 44x44px<br>• Badge tap emits `e.stopPropagation()`<br>• Drawer sheet uses natural height without vertical scrolling<br>• Document `window.scrollY` unchanged before and after opening drawer                                                                                |
